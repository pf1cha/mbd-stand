...
class AllGatherEvent(Event):
    def __init__(self, applying_time, handler, gpu_cards, data_size, method, steps=None, delta=None):
        ...
        self.delta = len(gpu_cards) // 2 if delta is None else delta
...
class AllGatherHandler(Handler):
    ...
    def do(self, event):
        if event.method == Method.RING:
            ...
            ring_handler = AllGatherHandler(self.future_event_list, self.clock)
            ...
            if event.steps == 1:
                return
            halving_handler = AllGatherHandler(self.future_event_list, self.clock)
            next_delta = event.delta // 2
            self.future_event_list.add_event(
                AllGatherEvent(applying_time, halving_handler,
                               event.gpu_cards, event.data_size, event.method,
                               steps=event.steps - 1,
                               delta=next_delta)
            )
